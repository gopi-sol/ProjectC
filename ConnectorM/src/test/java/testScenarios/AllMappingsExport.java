...
public class AllMappingsExport extends BaseClass {
	...
	static LinkedList<String> mappingsList = null;

	@BeforeSuite
	public void startTest() {

		File f = new File("reports\\ExtentReportsResults_Migr2.html");
	...
	static List<WebElement> mappingsCount, mapping = null;
	...
	public void mappingExport() throws IOException, InterruptedException {
		...
		String ExcelName = "TestMappingExportAuto.xlsx";

		// homePage.clickMappingCleanUpLogging();
		// driver.switchTo().frame("mainwindow");
		// utils.waitForElementToBeClickable(driver.findElement(By.xpath("//div[@id='headerdiv']")),
		// 30);
		...
		//
		// utils.waitForElementToBeClickable(mCLPage.getMappingCleanUpRows(),
		// 60);
		// mappingCleanUpRows =
		// driver.findElements(mCLPage.getMappingCleanUpRows());
		...
		//
		// mappingCleanUpColumns =
		// mappingCleanUpRows.get(i).findElements(By.tagName("td"));
		//
		...
		//
		// if
		// (mappingCleanUpColumns.get(0).getText().contains(mCLPage.currentDate()))
		// {
		...
		//
		// System.out.println(mappingCleanUpColumns.get(1) + " is about to be
		// clicked");
		//
		// WebElement toClick =
		// mappingCleanUpColumns.get(1).findElement(By.tagName("a"));
		////
		// }else {
			...
			if (utils.getElement(homePage.getPopupSelect1st()).getText().equals("No data found")
				...
				if (utils.getElement(homePage.getUserName()).getText().equals(",")) {
					...
					if (utils.getElement(mappingPage.getPopupSelect1st()).getText().equals("No data found") || utils
						...
						String totalMappingsCountText = driver
								.findElement(By.xpath("//div[@class='browserPagingInfo']/span[4]")).getText();
						int totalMappingsCount = Integer.parseInt(totalMappingsCountText);

						int noOfRepeat = totalMappingsCount / 10;

						int round = Math.round(noOfRepeat);

						System.out.println(round);//span[contains(text(),'View')]
						...
						int pageCount = Integer
								.parseInt(driver.findElement(By.xpath("//span[@id='lblTotPageCount']")).getText());

						System.out.println(pageCount);

						for (int j = 0; j < pageCount; j++) {

							List<WebElement> mappingsCount = driver.findElements(MappingPage.getMappingsInPopup());
//							String totalMappingsCountTexto = driver
//									.findElement(By.xpath("//span[@id='gvBrowse_ctl13_lblPageCount']")).getText();
//							int totalMappingsCounto = Integer.parseInt(totalMappingsCountTexto);
							System.out.println(mappingsCount.size());
							LinkedList<String> mappingsList = new LinkedList<String>();

							for (int k2 = 0; k2 < mappingsCount.size()-2; k2++) {
								WebElement mapping = mappingsCount.get(k2 + 1).findElement(By.tagName("td"));
								String mappingName = mapping.getText();
								mappingsList.add(mappingName);
								System.out.println(mappingName);
							}

							for (int k = 0; k < mappingsCount.size() - 2; k++) {

								if (k < 9) {
									System.out.println(k + 1);

									utils.waitForElementToBeClickable(mappingPage.getMappingSearchBox(), 90);
									utils.getElement(mappingPage.getMappingSearchBox()).sendKeys(mappingsList.get(k));
									utils.getElement(mappingPage.getMappingSearchBtn()).click();
									...
									test.log(LogStatus.PASS,
											...
											test.addScreenCapture(getAScreenshot()) + mappingsList.get(k)
													+ "- clicked");
									...
									try {
										...
										File fm = new File(
												System.getProperty("user.dir") + File.separator + "Mappings_Migr");
										fm.mkdir();

										String mandantFolder = tu.getData(ExcelName, "Mapping templates ", i + 1, 0)
												.replace("/", "|");

										File f = new File(System.getProperty("user.dir") + File.separator + "Mappings_Migr"
										...
										fileO = new StringSelection(
												absolutePathm + File.separator + mappingsList.get(k));
										...
										rb.setAutoDelay(2000); // Similar to
																// thread.sleep
									...
									utils.waitForElementToBeClickable(MappingPage.getNextpage(), 30);
									if (j<pageCount-1) {
									...
									for (int k2 = 0; k2 <(j+1); k2++) {
										...
										utils.waitForElementToBeClickable(MappingPage.getNextpage(), 30);
										utils.doJsClick(MappingPage.getNextpage());
									}
									

									System.out.println(MappingPage.getNextpage());
						...
						// if
						// (utils.getElement(mappingPage.getPopupSelect1st()).getText()
						// .equals(tu.getData(ExcelName, "Mapping templates ", i
						// + 1, 1))) {
						//
						// utils.getElement(mappingPage.getPopupSelect1st()).click();
						// driver.switchTo().window(parentWindowHandler);
						// driver.switchTo().frame("mainwindow");
						//
						// utils.waitForElementToBeVisible(mappingPage.getExportBtn(),
						// 90);
						// utils.getElement(mappingPage.getExportBtn()).click();
						// Thread.sleep(8000);
						//
						// File fm = new File(System.getProperty("user.dir") +
						// File.separator + "Mappings");
						// fm.mkdir();
						//
						// String mandantFolder = tu.getData(ExcelName, "Mapping
						// templates ", i + 1, 0).replace("/",
						// "|");
						//
						...
						// String absolutePathm = f.getAbsolutePath();
						//
						// String mappingName = tu.getData(ExcelName, "Mapping
						// templates ", i + 1, 1).replace("/",
						// "|");
						//
						// fileO = new StringSelection(absolutePathm +
						// File.separator + mappingName);
						//
						// Toolkit.getDefaultToolkit().getSystemClipboard().setContents(fileO,
						// null);
						//
						// Robot rb = null;
						//
						...
						//
						// test.addScreenCapture(getAScreenshot())
						// + tu.getData(ExcelName, "Mapping templates ", i + 1,
						// 1)
						// + "- mapping downloaded");
						//
						// driver.switchTo().defaultContent();
						...
						// TimeUnit.SECONDS);
						// driver.get(tu.getData("TestMappingExportAuto.xlsx",
						// "Sharepoint", 1, 0));
						// Thread.sleep(5000);
						// driver.manage().timeouts().implicitlyWait(120,
						// TimeUnit.SECONDS);
						...
						//
						// if (SharepointLoginpagetitle.equals("Sign in to your
						// account")) {
						// utils.waitForElementToBeClickable(driver.findElement(sharepointPage.getUsername()),
						// 30);
						// driver.findElement(sharepointPage.getUsername())
						// .sendKeys(tu.getData("TestMappingExportAuto.xlsx",
						// "Sharepoint", 1, 1));
						// System.out.println(tu.getData("TestMappingExportAuto.xlsx",
						// "Sharepoint", 1, 1));
						...
						// driver.manage().timeouts().implicitlyWait(30,
						// TimeUnit.SECONDS);
						// utils.waitForElementToBeClickable(driver.findElement(sharepointPage.getPassword()),
						// 30);
						...
						// System.out.println(tu.getData("TestMappingExportAuto.xlsx",
						// "Sharepoint", 1, 2));
						// utils.waitForElementToBeClickable(driver.findElement(sharepointPage.getSubmit()),
						// 30);
						// driver.findElement(sharepointPage.getSubmit()).click();
						// //
						// // Thread.sleep(10000);
						...
						//
						// }
						// utils.waitForElementToBeClickable(sharepointPage.getMyFiles(),
						// 120);
						// //
						...
						// 90);
						// folders =
						// driver.findElements(sharepointPage.getFolders());
						//
						// for (int k = 0; k < folders.size(); k++) {
						// System.out.println(folders.get(k).getText());
						// if (folders.get(k).getText().equals("Test")) {
						// System.out.println(folders.get(k).getText() + " is
						// about to be clicked");
						//
						// folders.get(k).click();
						...
						// utils.waitForElementToBeClickable(driver.findElement(sharepointPage.getFolder()),
						// 90);
						// driver.findElement(sharepointPage.getFolder()).click();
						// Thread.sleep(10000);
						// driver.manage().timeouts().implicitlyWait(120,
						// TimeUnit.SECONDS);
						// utils.waitForElementToBeClickable(sharepointPage.getFolderName(),
						// 90);
						// driver.findElement(sharepointPage.getFolderName())
						// .sendKeys("test_" + tu.getData(ExcelName, "Mapping
						// templates ", i + 1, 0));
						...
						// .equals("test_" + tu.getData(ExcelName, "Mapping
						// templates ", i + 1, 0))) {
						//
						...
						// "test_" + tu.getData(ExcelName, "Mapping templates ",
						// i + 1, 0));
						// utils.waitForElementToBeClickable(
						// driver.findElement(By.xpath("//span[contains(text(),'Create')]")),
						// 90);
						...
						// if (folders.size() == 0) {
						// utils.waitForElementToBeClickable(
						// driver.findElement(By.xpath("//span[contains(text(),'New')]")),
						// 90);
						// driver.findElement(By.xpath("//span[contains(text(),'New')]")).click();
						// utils.waitForElementToBeClickable(
						// driver.findElement(By.xpath("//span[contains(text(),'Folder')]")),
						// 90);
						...
						// TimeUnit.SECONDS);
						// utils.waitForElementToBeClickable(
						// driver.findElement(By.xpath("//input[@aria-label='Enter
						// your folder name']")),
						// 90);
						//
						// driver.findElement(By.xpath("//input[@aria-label='Enter
						...
						//
						// if (folders.get(k2).getText().equals("MappingExport_"
						// + dateStamp)) {
						//
						...
						// System.out.println(folders.size());
						// for (int k = 0; k < folders.size(); k++) {
						// if (folders.get(k).getText().equals("MappingExport_"
						// + dateStamp)) {
						//
						// System.out.println(folders.get(k).getText() + " is
						// about to be clicked");
						...
						// break;
						//
						// } else if (!("MappingExport_" +
						// dateStamp).equals(folders.get(k).getText())
						// && k == folders.size() - 1) {
						//
						// utils.waitForElementToBeClickable(
						// driver.findElement(By.xpath("//span[contains(text(),'New')]")),
						...
						// By.xpath("//input[@aria-label='Enter your folder
						// name']")), 90);
						//
						// driver.findElement(By.xpath("//input[@aria-label='Enter
						// your folder name']"))
						...
						//
						// folders.get(k2).click();
						// System.out.println("MappingExport_" + dateStamp +" is
						// created and Clicked");
						// Thread.sleep(3000);
						// break;
						// }
						...
						//
						// utils.waitForElementToBeClickable(
						// driver.findElement(By.xpath("//span[contains(text(),'Upload')]")),
						// 90);
						// driver.findElement(By.xpath("//span[contains(text(),'Upload')]")).click();
						// utils.waitForElementToBeClickable(
						// driver.findElement(By.xpath("//span[contains(text(),'Files')]")),
						// 90);
						...
						//
						// loginPage.ClearUsPwd();
						// loginPage.LoginPageTest(tu.getData("TestMappingExportAuto.xlsx",
						// "Login", 1, 1),
						// tu.getData("TestMappingExportAuto.xlsx", "Login", 1,
						// 2));
						//
						// try {
						//
						...
						//
						// driver.switchTo().defaultContent();
						// utils.waitForElementToBeClickable(homePage.getMandantBrowser(),
						// 90);
						// utils.getElement(homePage.getMandantBrowser()).click();
						// driver.switchTo().frame(utils.getElement(homePage.getPopupFrame()));
						// utils.getElement(homePage.getMandantSearchBox())
						// .sendKeys(tu.getData(ExcelName, "Mapping templates ",
						// i + 1, 0));
						// System.out.println(
						// tu.getData(ExcelName, "Mapping templates ", i + 1, 0)
						// + "is Passed");
						// utils.getElement(homePage.getMandantSearchBtn()).click();
						//
						// utils.waitForElementToBeClickable(homePage.getPopupSelect1st(),
						// 90);
						// utils.getElement(homePage.getPopupSelect1st()).click();
						// test.log(LogStatus.PASS,
						// test.addScreenCapture(getAScreenshot())
						...
						// 0)
						// + " -Mandant delegated");
						// System.out.println(tu.getData(ExcelName, "Mapping
						// templates ", i + 1, 0));
						// Thread.sleep(2000);
						//
						// homePage.clickMapping();
						// System.out.println("Mapping Clicked");
						// driver.switchTo().frame("mainwindow");
						// utils.waitForElementToBeClickable(mappingPage.getPageHeader(),
						// 90);
						// System.out.println(utils.getElement(mappingPage.getPageHeader()).getText());
						// utils.getElement(mappingPage.getPageHeader()).click();
						// System.out.println(
						// utils.getElement(mappingPage.getPageHeader()).getText()
						// + " is Clicked");
						//
						// utils.waitForElementToBeClickable(mappingPage.getMappingBrowser(),
						// 90);
						// utils.getElement(mappingPage.getMappingBrowser()).click();
						// String parentWindowHandler1 =
						// driver.getWindowHandle();
						// driver.switchTo().frame(utils.getElement(mappingPage.getPopupFrame()));
						// utils.waitForElementToBeClickable(mappingPage.getMappingSearchBox(),
						// 90);
						// utils.getElement(mappingPage.getMappingSearchBox())
						// .sendKeys(tu.getData(ExcelName, "Mapping templates ",
						// i + 1, 1));
						// utils.getElement(mappingPage.getMappingSearchBtn()).click();
						// Thread.sleep(2000);
						// utils.waitForElementToBeClickable(mappingPage.getPopupSelect1st(),
						// 90);
						//
						// utils.getElement(mappingPage.getPopupSelect1st()).click();
						// driver.switchTo().window(parentWindowHandler1);
						// driver.switchTo().frame("mainwindow");
						// utils.waitForElementToBeClickable(mappingPage.getDeleteBtn(),
						// 90);
						...
						//
						// } else if
						// (!mappingName.equals(folders.get(k).getText()) && k
						// == folders.size() - 1) {
						//
						// System.out.println(mappingName);
